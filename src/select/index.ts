...
export const createElSelectTestUtils = (wrapper: VueWrapper) => {
  const getSelectDomWrapper = <T extends Element = Element>(selector: string) => {
    let selectWrapper = wrapper.find<T>(selector);
  ...
  const getOptions: () => HTMLElement[] = () => Array.from(document.querySelectorAll<HTMLElement>("body > div:last-child .el-select-dropdown__item"));

  const getOptionLabels = (params: { onlyDisabled?: boolean } = {}) => {
  ...
  const getOption = (valueOrIndex: string | number) => {
    const options = getOptions();
  ...
  const getSelectVmWrapper = (selector: string) => {
    const selectWrapper = getSelectDomWrapper(selector);
    return selectWrapper.findComponent({ name: "ElSelect" });
  };

  const getValue = async (selector: string) => {
    await rAF();
    const selectComponentWrapper = getSelectVmWrapper(selector);
    return selectComponentWrapper.vm.modelValue;
  };

  const inputValue = async (value: string, selector: string) => {
    const selectComponentWrapper = getSelectVmWrapper(selector);
  ...
  return {
    ...
    getOptionDisabled(valueOrIndex: string | number) {
      const option = getOption(valueOrIndex);
      return option.classList.contains("is-disabled");
    },
    ...
    async select(valueOrIndex: string | number, selector: string) {
      ...
      const targetSelectOption = getOption(valueOrIndex);
    ...
    async clear(selector: string) {
      ...
      await rAF();
      const closeIconWrapper = wrapper.find(".el-select__caret.el-select__icon");
    ...
    async createValue(value: string, selector: string) {
      const selectComponentWrapper = getSelectVmWrapper(selector);
      ...
      await inputValue(value, selector);

      const options = getOptions().filter((t) => t.style.display !== "none");
...
export class ElSelectTestUtils extends BaseTestHelper {
  ...
  getSelectDomWrapper = <T extends Element = Element>(selector: string) => {
  ...
  getSelectVmWrapper = (selector: string) => {
