...
function generateModifiers(modifiers: KeyName[], isOnClick: boolean) {
  ...
  for (let i = 0;i < modifiers.length;i++) {
    ...
    if (
      systemKeyModifiers.includes(modifier as Exclude<typeof modifier, KeyName>)
      || (mouseKeyModifiers.includes(modifier as Exclude<typeof modifier, KeyName>)
        && isOnClick)
    ) {
...
export {
  TriggerOptions, createDOMEvent, keyCodesByKeyName, KeyName,
};
